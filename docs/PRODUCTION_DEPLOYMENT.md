...
### Ο εύκολος τρόπος (συνιστάται)

Ένα script παράγει το `.env.production` με έτοιμα, ισχυρά μυστικά. Ρωτά μόνο τις δύο δημόσιες διευθύνσεις — τις μόνες τιμές που δεν μπορεί να μαντέψει — και συμπληρώνει τα υπόλοιπα σωστά, συμπεριλαμβανομένου του `DATABASE_URL`, που είναι το σημείο όπου γίνονται τα περισσότερα λάθη:
...
Ή δίνοντας τις διευθύνσεις απευθείας:

```bash
./scripts/init-env.sh --admin-url https://vacuum-admin.example.com --api-url https://vacuum-api.example.com
```

Το script αρνείται να αντικαταστήσει υπάρχον `.env.production`: τα μυστικά του χρησιμοποιούνται ήδη από τη βάση και την αποθήκευση, και η αντικατάστασή τους θα κλείδωνε την εφαρμογή έξω από τα ίδια της τα δεδομένα.

Μετά την παραγωγή, **κρατήστε αντίγραφο του αρχείου σε ασφαλές σημείο** μαζί με τα υπόλοιπα μυστικά παραγωγής.

### Ο χειροκίνητος τρόπος

Αν προτιμάτε να ορίσετε εσείς τις τιμές, αντιγράψτε το πρότυπο και αντικαταστήστε κάθε `CHANGE_ME`:
