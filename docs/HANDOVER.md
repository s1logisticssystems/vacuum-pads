...
2. Εκτελούν `./scripts/init-env.sh` (ή `.\scripts\init-env.ps1` σε Windows), που παράγει το `.env.production` με έτοιμα μυστικά. Ρωτά μόνο τις δύο δημόσιες διευθύνσεις.
3. Ξεκινούν τις υπηρεσίες (`docker compose -f docker-compose.prod.yml ...`).
4. Δημιουργούν τον πρώτο διαχειριστή. **Χωρίς αυτό κανείς δεν μπορεί να συνδεθεί.**
...
Τα βήματα 1 έως 5 περιγράφονται ως αριθμημένα βήματα στο `README.md`· το βήμα 4 αναλυτικά στο `USER_MANAGEMENT.md`.

Το `.env.production` που παράγεται στο βήμα 2 περιέχει τον κωδικό της βάσης και το κλειδί υπογραφής. **Πρέπει να κρατηθεί αντίγραφό του** σε ασφαλές σημείο: αν χαθεί, η εφαρμογή δεν μπορεί να προσπελάσει τα δεδομένα της.
...
- [ ] Το `.env.production` έχει παραχθεί στον διακομιστή τους και έχει κρατηθεί αντίγραφό του.
