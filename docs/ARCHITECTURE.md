...
| Μέρος | Γλώσσα | Γραμμές | Αρχεία |
|---|---|---|---|
| API | TypeScript | ~25.900 | 92 |
| Περιβάλλον διαχείρισης | TypeScript (TSX) | ~9.900 | 4 |
| Εφαρμογή Android | Dart | ~12.500 | 33 |

**Το API και το περιβάλλον διαχείρισης χρησιμοποιούν την ίδια γλώσσα.** Αυτό δεν είναι σύμπτωση: οι ίδιοι ορισμοί τύπων διατρέχουν και τα δύο, οπότε μια αλλαγή στο σχήμα μιας απάντησης του API εντοπίζεται κατά τη μεταγλώττιση του περιβάλλοντος διαχείρισης και όχι από τον χρήστη. Πρακτικά σημαίνει επίσης ότι η συντήρηση του συστήματος απαιτεί ένα σύνολο δεξιοτήτων αντί για δύο.

Το TypeScript μεταγλωττίζεται σε JavaScript και εκτελείται στο Node.js 22. Στον διακομιστή δεν εγκαθίσταται τίποτα από αυτά — τρέχουν μέσα στα containers.

Το Dart χρησιμοποιείται αποκλειστικά για την εφαρμογή Android, όπου το Flutter παράγει native κώδικα. Δεν εκτελείται πουθενά στην υποδομή.

### Υποστηρικτικά αρχεία

Πέρα από τον κώδικα της εφαρμογής υπάρχουν:

| Είδος | Ρόλος |
|---|---|
| SQL | Μεταβάσεις της βάσης. Παράγονται από το Prisma και ελέγχονται πριν την εφαρμογή τους. |
| CSS | Εμφάνιση του περιβάλλοντος διαχείρισης. |
| Bash και PowerShell | Scripts λειτουργίας: αντίγραφα ασφαλείας, παραγωγή ρυθμίσεων, διαχείριση κωδικών. Υπάρχουν και στις δύο μορφές ώστε να μη δεσμεύεται η επιλογή λειτουργικού συστήματος. |
| Dockerfile / Compose | Ορισμός των εικόνων και της σύνθεσης των υπηρεσιών. |
...
Όλα τα μέρη τρέχουν σε containers μέσω `docker-compose.prod.yml`. Η PostgreSQL και το MinIO δεν εκθέτουν καμία θύρα προς τα έξω· backend και διαχείριση δένονται στο `127.0.0.1` ώστε ο δικός σας reverse proxy να αναλαμβάνει το HTTPS και να εκθέτει μόνο τις εγκεκριμένες διαδρομές.
